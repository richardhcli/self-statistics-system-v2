...
The user's global level and attribute breakdown are managed by the **Player Statistics store** and the `@self-stats/progression-system` module. The Statistics dashboard (Status and Level views) displays level, EXP, and attribute data sourced from `usePlayerStatistics()`. See [features-statistics.md](./features-statistics.md) and [ai-and-gamification.md](../ai-and-gamification.md) for details.